...
# -----------------------------------------------------------------------------
# Quote request flow: Streamlit -> n8n webhook -> Odoo
# -----------------------------------------------------------------------------
...
with st.expander("Generate quote", expanded=False):
    ...
    access_granted = bool(configured_access_code) and access_code == configured_access_code

    if access_code and not access_granted:
        ...
        with st.form("quote_request_form", clear_on_submit=False):
            ...
            with q1:
                company_name = st.text_input(
                    "Company name",
                    placeholder="As detailed and correct as possible",
                )
            ...
            if submitted:
                ...
                if not contact_name.strip():
                    missing_fields.append("Contact person name")
                if not contact_email.strip():
                    missing_fields.append("Contact person email")
                elif not is_valid_email(contact_email):
                    missing_fields.append("Valid contact person email")
                ...
                if missing_fields:
                    ...
                    payload = {
                        ...
                        "scenario": {
                            ...
                            "daily_footfall_per_store": float(visitors_day),
                            "conversion_rate_pct": float(conv_pct * 100),
                            "average_ticket_value": float(atv),
                            "gross_margin_pct": float(gross_margin * 100),
                            "annual_footfall_per_store": float(visitors_year_store),
                            "annual_footfall_total": float(visitors_year_store * n_stores),
                            "conversion_uplift_pp": float(conversion_uplift * 100),
                            "new_conversion_rate_pct": float(conv_pct_new * 100),
                            "new_atv": float(atv_new),
                            "atv_uplift_pct": float(atv_uplift * 100),
                            "footfall_uplift_pct": float(footfall_uplift * 100),
                            "open_days_per_week": open_days,
                            "contract_term_years": tco_years,
                            "optional_saturday_scenario": {
                                "sat_share_pct": sat_share * 100,
                                "sat_conversion_boost_pct": sat_boost * 100,
                            },
                            "commercial_assumptions": {
                                "install_cost_per_store": install_cost_store,
                                "monthly_subscription_per_store": monthly_service_cost_store,
                                "tco_per_store": tco_store,
                                "tco_total": tco_total,
                            },
                            "preset": {
                                "name": preset_name,
                                "description": st.session_state.get("preset_desc", ""),
                            },
                        },
                        "results": {
                    ...
                    try:
                        ...
                        st.error("Quote request could not be sent.")
                        st.caption(str(exc))
